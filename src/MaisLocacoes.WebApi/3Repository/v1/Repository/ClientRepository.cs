...
namespace Repository.v1.Repository
{
    public class ClientRepository : IClientRepository
    {
        ...
        public async Task<IEnumerable<ClientEntity>> GetClientsByPage(int items, int page, string query)
        {
            using var context = _contextFactory.CreateContext();
            if (query == null)
                return await context.Set<ClientEntity>().Include(c => c.Address).Where(c => c.Status != ClientStatus.ClientStatusEnum.ElementAt(2) /*inactive*/).Skip((page - 1) * items).Take(items).ToListAsync();
            else
                return await context.Set<ClientEntity>().Include(c => c.Address).Where(c => (
                     ...
                     c.Email.ToLower().Contains(query.ToLower())) && c.Status != ClientStatus.ClientStatusEnum.ElementAt(2) /*inactive*/)
