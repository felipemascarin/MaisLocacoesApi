...
namespace MaisLocacoes.WebApi._3Repository.v1.Repository
{
    public class ContractRepository : IContractRepository
    {
        ...
        public async Task<ContractEntity> CreateContract(ContractEntity contractEntity)
        {
            ...
            await context.Contracts.AddAsync(contractEntity);
            context.SaveChanges();
        ...
        {
            using var context = _contextFactory.CreateContext();
            return (await context.Contracts.OrderByDescending(c => c.Version).FirstOrDefaultAsync(c => c.RentId == rentId)).Version;
        }

        public async Task<ContractEntity> GetTheLastContract(int rentId)
        {
            using var context = _contextFactory.CreateContext();
            return await context.Contracts.OrderByDescending(c => c.Version).FirstOrDefaultAsync(c => c.RentId == rentId);
        }

        public async Task<int> UpdateContract(ContractEntity contractForUpdate)
        {
            using var context = _contextFactory.CreateContext();
            context.Contracts.Update(contractForUpdate);
