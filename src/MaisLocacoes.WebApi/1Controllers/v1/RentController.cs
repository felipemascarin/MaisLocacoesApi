...
namespace MaisLocacoes.WebApi.Controllers.v1
{
    ...
    public class RentController : Controller
    {
        ...
        public async Task<IActionResult> CreateRent([FromBody] CreateRentRequest rentRequest)
        {
            try
            {
                _logger.LogInformation("CreateRent {@dateTime} {@rentRequest} User:{@email} Cnpj:{@cnpj}", TimeZoneInfo.ConvertTimeFromUtc(System.DateTime.UtcNow, _timeZone), JsonConvert.SerializeObject(rentRequest), _email, _schema);
        ...
        public async Task<IActionResult> GetAllByClientId(int clientId)
        {
            try
            {
                _logger.LogInformation("GetAllByClientId {@dateTime} clientId:{@clientId} User:{@email} Cnpj:{@cnpj}", TimeZoneInfo.ConvertTimeFromUtc(System.DateTime.UtcNow, _timeZone), clientId, _email, _schema);
        ...
        public async Task<IActionResult> GetRentsByPage(int items, int page, [FromQuery(Name = "query")] string query, [FromQuery(Name = "status")] string status)
        {
            try
            {
                _logger.LogInformation("GetRentsByPage {@dateTime} items:{@items} pages:{@page} query:{@query} User:{@email} Cnpj:{@cnpj}", TimeZoneInfo.ConvertTimeFromUtc(System.DateTime.UtcNow, _timeZone), items, page, query, _email, _schema);
        ...
        public async Task<IActionResult> UpdateRent([FromBody] UpdateRentRequest rentRequest, int id)
        {
            try
            {
                _logger.LogInformation("UpdateRent {@dateTime} {@rentRequest} id:{@id} User:{@email} Cnpj:{@cnpj}", TimeZoneInfo.ConvertTimeFromUtc(System.DateTime.UtcNow, _timeZone), JsonConvert.SerializeObject(rentRequest), id, _email, _schema);
